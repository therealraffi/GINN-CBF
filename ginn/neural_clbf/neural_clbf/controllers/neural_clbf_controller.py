...
class NeuralCLBFController(pl.LightningModule, CLFController):
    ...
    def V_with_jacobian(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.dynamics_model.angle_dims:
    ...
    def training_step(self, batch, batch_idx):
        ...
        descent_loss = self.descent_loss(x, goal_mask, safe_mask, unsafe_mask, requires_grad=True)
        component_losses.update(
            descent_loss
        )
        descent_loss_component = {}
        descent_loss_component.update(descent_loss)
        print(descent_loss)
        print("Components:", descent_loss_component)
        ...
        total_descent_loss = 0
        for _, loss_value in descent_loss_component.items():
            if not torch.isnan(loss_value): 
                total_descent_loss = total_descent_loss + loss_value

        print("total:", total_loss)
        print("total descent:", total_descent_loss)
