...
from GINN.geometry.sampling_primitives import BBLineSampler, sample_bbox, sample_disk, sample_line_segment
...
class SampleConstraint(Constraint):
    ...
    def get_sampled_points(self, N):
        # print("------------ SAMPLING ------------")
        # print(self.current_idx, self.current_idx + N)
        if self.current_idx + N > self.sample_pts.shape[0]:
            self.current_idx = 0
            self.shuffled_idcs = torch.randperm(self.sample_pts.shape[0])
        # print(self.shuffled_idcs.shape, self.sample_pts.shape[0])
        sampled_pts = self.sample_pts[self.shuffled_idcs[self.current_idx:self.current_idx + N]]
        self.current_idx += N
        return sampled_pts

class SampleConstraintWithTargets(Constraint):
    def __init__(self, sample_pts, target_pts):
        self.sample_pts = sample_pts
    ...
    def get_sampled_points(self, N):
        if self.current_idx + N > self.sample_pts.shape[0]:
            ...
            self.shuffled_idcs = torch.randperm(self.sample_pts.shape[0])
        sampled_pts = self.sample_pts[self.shuffled_idcs[self.current_idx:self.current_idx + N]]
        target_pts = self.target_pts[self.shuffled_idcs[self.current_idx:self.current_idx + N]]
        self.current_idx += N
        return sampled_pts, target_pts

class SampleConstraintinPairs(Constraint):
    def __init__(self, sample_pts):
        self.sample_pts = sample_pts
        perm = torch.randperm(self.sample_pts.shape[0] * 2) 
        self.shuffled_idcs = perm.view(2, self.sample_pts.shape[0])
        self.current_idx = 0
        print(self.shuffled_idcs.shape)
    ...
    def get_sampled_points(self, N):
        if self.current_idx + N > self.sample_pts.shape[0]:
            ...
            perm = torch.randperm(self.sample_pts.shape[0] * 2) 
            self.shuffled_idcs = perm.view(2, self.sample_pts.shape[0])
        start_pts = self.sample_pts[self.shuffled_idcs[0]][self.current_idx:self.current_idx + N]
        goal_pts = self.sample_pts[self.shuffled_idcs[1]][self.current_idx:self.current_idx + N]
        self.current_idx += N
        return start_pts, goal_pts
...
class SampleConstraintsWithControls(Constraint):
    def __init__(self, sample_pts, controls):
        assert sample_pts.shape[0] == controls.shape[0]
        self.sample_pts = sample_pts
        self.controls = controls
        perm = torch.randperm(self.sample_pts.shape[0]) 
        self.shuffled_idcs = perm.view(self.sample_pts.shape[0])
        self.current_idx = 0
    ...
    def get_sampled_points(self, N):
        if self.current_idx + N > self.sample_pts.shape[0]:
            ...
            perm = torch.randperm(self.sample_pts.shape[0]) 
            self.shuffled_idcs = perm.view(self.sample_pts.shape[0])
        start_pts = self.sample_pts[self.shuffled_idcs][self.current_idx:self.current_idx + N]
        controls = self.controls[self.shuffled_idcs][self.current_idx:self.current_idx + N]
        self.current_idx += N

        return start_pts, controls
