...
class CriticalPointsHelper():
    ...
    def _gd_to_min_grad_norm(self, z, pc: PointWrapper, lr, epoch=None, early_stop=False, plot_descent=False, **kwargs):
        ...
        opt = torch.optim.Adam([pc.data], lr=1e-4)
        self.logger.info("lr: " + str(opt.param_groups[-1]['lr']))
        ...
        for i in (pbar := trange(max_iter)):
            ...
            # Compute the gradient norms before clipping
            grad_norms = pc.data.grad.norm(2, dim=1)  # Compute L2 norm of gradients for each point
            max_grad_norm = grad_norms.max().item()  # Greatest gradient norm
            avg_grad_norm = grad_norms.mean().item()  # Average gradient norm

            # self.logger.info(f"Greatest gradient norm before clipping: {max_grad_norm}")
            # self.logger.info(f"Average gradient norm before clipping: {avg_grad_norm}")
            # self.logger.info(f"=============================================================================")

            # # Clip gradients to avoid large updates
            torch.nn.utils.clip_grad_norm_(pc.data, max_norm=1.0)
