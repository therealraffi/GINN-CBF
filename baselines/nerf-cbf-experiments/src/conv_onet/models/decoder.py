...
class MLP(nn.Module):
    ...
    def sample_grid_feature(self, p, c):
        ...
        c = F.grid_sample(c.to(device), vgrid.to(device), padding_mode='border', align_corners=True,
    ...
    def forward(self, p, c_grid=None):
        ...
        embedded_pts = self.embedder(p).to(device)
        h = embedded_pts
        for i, l in enumerate(self.pts_linears):
            h = self.pts_linears[i].to(device)(h)
            h = F.relu(h)
            if self.c_dim != 0:
                h = h + self.fc_c[i].to(device)(c)
            if i in self.skips:
                h = torch.cat([embedded_pts, h], -1)
        out = self.output_linear.to(device)(h)
        if not self.color:
            out = out.squeeze(-1)
        return out
...
class MLP_no_xyz(nn.Module):
    ...
    def sample_grid_feature(self, p, grid_feature):
        p_nor = normalize_3d_coordinate(p.clone(), self.bound)
        p_nor = p_nor.unsqueeze(0)
        vgrid = p_nor[:, :, None, None].float().to(device)
        c = F.grid_sample(grid_feature.to(device), vgrid, padding_mode='border',
