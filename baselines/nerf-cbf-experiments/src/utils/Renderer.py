...
class Renderer(object):
    ...
    def render_batch_img(self, c, decoders, c2w, device, stage, gt_depth=None):
        """
        Renders out depth, uncertainty, and color images.
        ...
        with torch.no_grad():
            ...
            for i in range(c2w.shape[0]):
                rays_o, rays_d = get_rays(H, W, self.fx, self.fy, self.cx, self.cy, c2w[i], device)
                rays_o = rays_o.reshape(-1, 3)
                rays_d = rays_d.reshape(-1, 3)
                ...
                if i == 0:
                    ...
                    batch_rays_o = torch.cat((batch_rays_o, rays_o), dim=0)
                    batch_rays_d = torch.cat((batch_rays_d, rays_d), dim=0)
            ...
            for i in range(0, batch_rays_d.shape[0], ray_batch_size):
                rays_d_batch = batch_rays_d[i:i+ray_batch_size]
                rays_o_batch = batch_rays_o[i:i+ray_batch_size]
                if gt_depth is None:
                    ret = self.render_batch_ray(
                        c, decoders, rays_d_batch, rays_o_batch, device, stage, gt_depth=None)
                else:
            ...
            color = torch.cat(color_list, dim=0)
            '''
            ret = self.render_batch_ray(c, decoders, batch_rays_d, batch_rays_o, device, stage, gt_depth=None)
            depth, uncertainty, color = ret
            '''
            depth = depth.reshape(c2w.shape[0], H, W)
            uncertainty = uncertainty.reshape(c2w.shape[0], H, W)
            color = color.reshape(c2w.shape[0], H, W, 3)
            return depth, uncertainty, color
